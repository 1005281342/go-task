module github.com/1005281342/go-task

go 1.16

require (
	github.com/1005281342/httproxy v0.0.0-20220323171555-42c0d8d46afb
	github.com/1005281342/log v0.0.4
	github.com/apolloconfig/agollo/v4 v4.1.0
	github.com/fullstorydev/grpchan v1.1.1
	github.com/gobuffalo/here v0.6.5 // indirect
	github.com/golang-jwt/jwt/v4 v4.4.0 // indirect
	github.com/google/uuid v1.3.0
	github.com/hako/durafmt v0.0.0-20210608085754-5c1018a4e16b // indirect
	github.com/hashicorp/errwrap v1.1.0 // indirect
	github.com/hashicorp/go-hclog v1.2.0 // indirect
	github.com/hashicorp/serf v0.9.7 // indirect
	github.com/hibiken/asynq v0.22.1
	github.com/json-iterator/go v1.1.12
	github.com/lufia/plan9stats v0.0.0-20220305071607-d0b38dbe16db // indirect
	github.com/magiconair/properties v1.8.6 // indirect
	github.com/polarismesh/polaris-go v1.0.1
	github.com/power-devops/perfstat v0.0.0-20220216144756-c35f1ee13d7c // indirect
	github.com/prometheus/client_golang v1.12.1
	github.com/rookie-ninja/rk-common v1.2.4 // indirect
	github.com/rookie-ninja/rk-entry v1.0.11 // indirect
	github.com/rookie-ninja/rk-entry/v2 v2.1.4
	github.com/rookie-ninja/rk-prom v1.1.4
	github.com/rookie-ninja/rk-zero v1.0.3
	github.com/shirou/gopsutil/v3 v3.22.2 // indirect
	github.com/spf13/afero v1.8.2 // indirect
	github.com/tklauser/go-sysconf v0.3.10 // indirect
	github.com/zeromicro/go-zero v1.3.1
	github.com/zeromicro/zero-contrib/zrpc/registry/polaris v0.0.0-20220228111653-d672d81f39ab
	go.opentelemetry.io/otel v1.6.0 // indirect
	golang.org/x/sys v0.0.0-20220319134239-a9b59b0215f8 // indirect
	google.golang.org/genproto v0.0.0-20220324131243-acbaeb5b85eb // indirect
	google.golang.org/grpc v1.45.0
	google.golang.org/protobuf v1.28.0
	gopkg.in/ini.v1 v1.66.4 // indirect
)
